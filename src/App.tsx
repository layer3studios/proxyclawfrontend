        ...
        <header className="sticky top-0 z-50 w-full border-b bg-background/80 backdrop-blur-lg">
          <div className="container mx-auto px-4 h-16 flex items-center justify-between">
            <motion.div className="flex items-center gap-2" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
              ...
              <span className="font-bold text-xl hidden sm:inline">{BRAND.name}</span>
            </motion.div>

            <motion.nav className="hidden md:flex items-center gap-4" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
              <Button variant="default"
                onClick={() => {
                  if (!hasActiveSub) { toast(`Subscribe to the ${BRAND.plan.name} plan first.`); return; }
                  setShowCreateForm(!showCreateForm);
                }}
                className="gap-2" disabled={!hasActiveSub}>
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" className="gap-2">
                    ...
                    <span className="max-w-[150px] truncate hidden lg:inline">{user?.email}</span>
                  </Button>
                </DropdownMenuTrigger>
            ...
            {showCreateForm ? (
              <motion.div key="create-form" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="mb-8">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <Button variant="ghost" size="icon" onClick={() => setShowCreateForm(false)}><X className="w-5 h-5" /></Button>
                </div>
